...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    ...
    final values = await Future.wait([repo.count(), repo.inventoryValue(), repo.lowStock()]);
    if (mounted) setState(() { _count = values[0] as int; _inventory = values[1] as double; _low = (values[2] as List).length; });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(onRefresh: _load, child: ListView(padding: const EdgeInsets.all(18), children: [
        ...
        Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [Text('الإصدار الأول', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)), SizedBox(height: 6), Text('قاعدة بيانات محلية، إدارة المنتجات، أسعار الشراء والبيع، الكميات، صور المنتجات، والبحث السريع. الخطوة التالية: شاشة المبيعات والفواتير والتقارير ومزامنة السحابة والتعرف الذكي بالصورة.')]))),
      ])),
    );
  }
